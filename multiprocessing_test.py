...
class Settings:
    ...
    output_directory = pathlib.Path("results")
...
def _get_initial_state(process_id, settings):
    ...
    initial_state = np.array(
        [
            rng.uniform(
                low=settings.parameter_ranges[i][0],
                high=settings.parameter_ranges[i][1],
            )
...
def _save_trace(process_id, mcmc_trace, settings):
    output_directory = settings.output_directory / pathlib.Path(
